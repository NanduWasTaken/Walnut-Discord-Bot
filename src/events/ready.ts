...
import { CommandData } from "../function"
...
export async function RegisterCommand(client: Client<true>) {
    ...
    const commands: Array<RESTPostAPIApplicationCommandsJSONBody | SlashCommandBuilder> = []
    const TestCommands: Array<RESTPostAPIApplicationCommandsJSONBody | SlashCommandBuilder> = []
    ...
    try {
        ...
        if (TestCommands.length > 0) {
            for (const x of config.guildId) {
                await rest
                    .put(Routes.applicationGuildCommands(client.user.id, x), {
                        body: TestCommands
                    })
                    .then(() =>

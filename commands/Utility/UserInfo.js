...
module.exports = {
    ...
    execute(interaction) {
        ...
        const name = !/^[a-z0-9._]+$/.test(member.user.username) ? `${member.user.tag}` : `@${member.user.username}`
            ...
            embeds: [
                {
                    ...
                    fields: [
                        ...
                        },
                        {
                            ...
                            value: `<t:${(member.user.createdAt.getTime() / 1000.0).toFixed()}:F> (<t:${(
                                member.user.createdAt.getTime() / 1000.0
                            ).toFixed()}:R>)`
                        },
                        {
                            name: "Joined Server",
                            value: `<t:${(member.joinedAt.getTime() / 1000.0).toFixed()}:F> (<t:${(

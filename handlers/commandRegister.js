...
module.exports = {
  ...
  const commands = await commandCompairing.compair(localCommands, applicationCommands);

  try {
    await client.application.commands.set(commands, guildId ? guildId : undefined);
    console.log(`[✅] Successfully registered ${localCommands.length} (/) commands ${mode}`);
  } catch (error) {
